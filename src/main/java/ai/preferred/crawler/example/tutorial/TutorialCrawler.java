...
public class TutorialCrawler {
  ...
  public static Crawler createCrawler() {
    ...
    final Crawler crawler = Crawler.buildDefault();
  ...
  public static Fetcher createFetcher() {
    ...
    AsyncFetcher.Builder b = AsyncFetcher.builder();
    EmptyContentValidator ecv = new EmptyContentValidator();
    StatusOkValidator sov = new StatusOkValidator();
    TutorialValidator tv = new TutorialValidator();
    b.validator(new PipelineValidator(ecv,sov,tv));
    
    
    // Create the fetcher here
    final Fetcher fetcher = b.build();
  ...
  public static Session createSession(List<Paper> papers) {
    ...
    b.put(PAPER_LIST_KEY, papers);  
    final Session session = b.build();
  ...
  public static Crawler createCrawler(Fetcher fetcher, Session session) {
    // Create the crawler here
    Crawler.Builder b = Crawler.builder();
    b.fetcher(fetcher);
    b.session(session);
    
    final Crawler crawler = b.build();
    crawler.start();
    return crawler;
  }
  ...
  public static void main(String[] args) throws Exception {
    ...
    try (final Crawler crawler = createCrawler(createFetcher(), createSession(papers))) {
        VRequest vreq = new VRequest("https://preferred.ai/publications");
        crawler.getScheduler().add(vreq, new TutorialHandler());
    }
